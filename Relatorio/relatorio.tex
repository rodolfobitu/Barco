...
O requisito para medição de velocidade de rotação (REQ4) foi cumprido com o bloco ``Cooler'' e no prório bloco ``ES670_pp''. O bloco ``cooler'' configura o hardware para a utilização do cooler e controla o estado ligado e desligado.
Ainda foi adicionado no ``ES670_pp''o cíclo cooperativo, então foram colocadas 3 tasks: a que controla o estado do cooler através do botão, a que calcula a velocidade (utlizando sensores infravermelho) e a que chama a máquina de estdos.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\linewidth]{esq_cooler}
	\caption{Esquema do componente LCD}
	\label{fig:maquina}
\end{figure}
\begin{figure}[H]
...
\section{Executivo cíclo cooperativo}

Para fazer o cíclo cooperativo, foram implementadas tasks para executar as tarefas que cumprem os requisitos.

\subsection{Cálculo da velocidade}
No arquivo es670_pp é chamado ``es670_computeCoolerVelocity", sua função é atualizar o valor da velocidade do cooler. Um vetor cíclico de 10 posições é usado para guardar o histórico e esse mesmo velor é usado para calcular a velocidade. Ainda é avisado para a Máquina de Estados o valor atual da velocidade.

\subsection{Controle do estado do ventilador}
No arquivo es670_pp é chamado de ``es670_coolerTask'', sua função é desligar ou ligar o cooler a partir da entrada que é fornecida a partir do botão apertado.
Futuramente, queremos remover essa task e adicioná-la à Máquina de Estados.

\subsection{Chamada para a Máquina de Estados}
No arquivo es670_pp é chamado de ``es670_commandMachineTask'', sua função é acionar a máquina de estados a partir de um buffer que é preenchino quando a interrupção é chamada a partir do serial.

\subsection{As interrupções}
O tratamento de interrupções pode ser visto no arquivo es670_pp em ``isr_CyclicExecutive''. As interrupções tratadas estão listadas abaixo:
\begin{itemize}
...
\item Envio serial: Essa interrupção é utiizada para enviar informações através da porta serial sem comprometer o cíclo cooperativo. Sempre que um caractere novo pode ser enviado, ele é enviado.
\end{itemize}
...
Tivemos problemas com relação ao tempo, pois tentamos implementar as três funções que foram pedidas inicialmente para a porta serial, mas só conseguimos colocar a função do LED para funcionar e a função do Buzzer estava quase pronta, mas não conseguimos a tempo. Colocamos no modelo, mas a implemntação estava incompleta. Depois, com mais tempo, voltamos e implementamos essa funcionalidade.

Na implentação do comando de exibição no LCD tivemos problema por não esperar algum tempo após enviar o comando de limpar o display para enviar os dados a serem exibidos. Sem essa espera, parte do conteúdo era ``engolido''.

Durante a implementação do cíclo cooperativo, reparamos que o restante da implemtação anterior não encaixava na idéia do cíclo, portanto precisamos fazer adaptações nas implementações anteriores para fazer que os requisitos anteriores continuassem a ser cumpridos. Apesar de termos conseguido resultados positivos, ainda não resolvermos o problema do buzzer, pois ele ainda está tomando o processo para si enquanto está ativo. Pretendemos usar um timer e sua interrupção para reseolver isso, mas ainda não vimos se ainda há um terceiro timer ou precisaremos criar alguma outra solução (timer2 está reservado para o pwm).
...
\begin{thebibliography}{widestlabel}
	...
	\bibitem{bb:github}{Código Fonte no\href{https://github.com/rodolfobitu/Barco/tree/bbf444209c75c920969b1e1d208ab825b013e332}{GitHub}}
